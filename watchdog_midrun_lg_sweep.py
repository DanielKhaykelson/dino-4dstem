"""watchdog_midrun_lg_sweep.py -- poll runs/_supcon_lg_sweep/ for new
ckpt_ep*.pth files and render midrun_ep*.png next to each, using
midrun_class_map.infer().

Runs alongside the training sweep; whenever a new ckpt is saved it
produces the matching map within the next poll cycle.

Stops automatically when STOP_SENTINEL exists at SWEEP_ROOT/_done.flag,
or when killed.
"""
...
SWEEP_ROOT = os.path.join("runs", "_supcon_lg_sweep")
POLL_SEC = 30
...
def render_one(ckpt_path: str, sample: str, out_png: str, device):
    ...
    axes[1].set_title(f"Ours: {label}  [{sample}/{os.path.basename(os.path.dirname(ckpt_path))}]",
                       fontsize=11)
...
def discover_pending():
    """Return list of (ckpt_path, sample, out_png) for ckpts that lack maps."""
    pending = []
    pattern = os.path.join(SWEEP_ROOT, "*", "*", "ckpt_ep*.pth")
    for ckpt_path in sorted(glob.glob(pattern)):
        # path = runs/_supcon_lg_sweep/<sample>/<label>/ckpt_epN.pth
...
def main():
    ...
    print(f"[watchdog] device={device}  sweep_root={SWEEP_ROOT}  "
          f"poll={POLL_SEC}s", flush=True)
    print(f"[watchdog] stop when {STOP_FLAG} exists or killed", flush=True)
    rendered_total = 0
    while True:
        if os.path.exists(STOP_FLAG):
            print(f"[watchdog] stop flag found, exiting "
                  f"(rendered {rendered_total})", flush=True)
            return
        try:
            pending = discover_pending()
        except Exception as e:
            print(f"[watchdog] discover error: {e!r}", flush=True)
            pending = []
        for ckpt_path, sample, out_png in pending:
            try:
                ...
                print(f"[watchdog] render error {ckpt_path}: {e!r}", flush=True)
        time.sleep(POLL_SEC)
