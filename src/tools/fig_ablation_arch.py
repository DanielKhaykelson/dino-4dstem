...
DISP = {"Na007b": "Na007b", "IMC_SI5": "indomethacin (magnified interface)"}
...
def classmap(ax, cell, title, idx=None, fs=10):
...
for ri, s in enumerate(["Na007b", "IMC_SI5"]):
    classmap(figA.add_subplot(2, 2, ri * 2 + 1), f"01_vit_vs_resnet/vit_{s}", f"{DISP[s]}: ViT encoder", ri * 2 + 0, fs=9.5)
    classmap(figA.add_subplot(2, 2, ri * 2 + 2), f"02_resnet_depth/{s}_L1", f"{DISP[s]}: ResNet (shallow)", ri * 2 + 1, fs=9.5)
figA.tight_layout(); FigureCanvasAgg(figA)
...
for ri, s in enumerate(["Na007b", "IMC_SI5"]):
    for ci, L in enumerate([1, 2, 3, 4]):
        classmap(figB.add_subplot(2, 4, ri * 4 + ci + 1), f"02_resnet_depth/{s}_L{L}", f"{DISP[s]}: depth L{L}", ri * 4 + ci, fs=8)
