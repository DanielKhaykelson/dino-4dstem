"""watchdog_midrun_c1d_sweep.py -- same as the lg-sweep watchdog but
points at runs/_dino_c1d_sweep/.
"""
...
SWEEP_ROOT = os.path.join("runs", "_dino_c1d_sweep")
POLL_SEC = 30
...
def main():
    ...
    print(f"[watchdog c1d] device={device}  sweep_root={SWEEP_ROOT}  "
          f"poll={POLL_SEC}s", flush=True)
    print(f"[watchdog c1d] stop when {STOP_FLAG} exists", flush=True)
    rendered_total = 0
    while True:
        if os.path.exists(STOP_FLAG):
            print(f"[watchdog c1d] stop flag found, exiting "
                  f"(rendered {rendered_total})", flush=True)
            return
        try:
            pending = discover_pending()
        except Exception as e:
            print(f"[watchdog c1d] discover error: {e!r}", flush=True)
            pending = []
        for ckpt_path, sample, out_png in pending:
            try:
                ...
                print(f"[watchdog c1d] render error {ckpt_path}: {e!r}", flush=True)
        time.sleep(POLL_SEC)
