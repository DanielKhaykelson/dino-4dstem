"""run_paper_K10_probe_commit.py — Probe-and-commit pipeline at K=10 across
all 5 samples for the paper.

Pipeline per sample:
    1. Train 5-epoch probe (gamma=0, K=10) -> probe/training_log.csv
    2. Read avg_conf at ep5 -> decide gamma_main (threshold 0.85)
    3. If gamma_main == 0: continue probe checkpoint to 50 ep, no waste
       If gamma_main == 1: discard probe, train fresh 50 ep with gamma=1
    4. Save probe_decision.json
    5. Run improved evaluation (better GradCAMs)

Output tree:
    dino_sr_contrastive/runs/_paper_K10_probe_commit/
        Na007b/
            probe/                 # 5-epoch gamma=0
            main/                  # 50-epoch with decided gamma
                best.pth
                eval/
                    fig_*.png
                    gradcam/
...
Sample order (longest first so that we make progress on key results early):
    1. EuInAs_B100   (~45 min, +weight)  - the canonical layered case
    2. IMC_50nm_SI2  (~25 min, +weight)  - NEW validation point
    3. Na007b        (~25 min, vanilla)
    4. Na006a        (~25 min, vanilla)
    5. IMC_150nm_SI5 (~30 min, vanilla)
"""
...
# Order: longest/most important first
SAMPLES = ["EuInAs_B100", "IMC_50nm_SI2", "Na007b", "Na006a", "IMC_150nm_SI5"]

OUT_ROOT = os.path.join("runs", "_paper_K10_probe_commit")
...
def main():
    ...
    print(f"[paper K=10 probe-and-commit] device={device}", flush=True)
